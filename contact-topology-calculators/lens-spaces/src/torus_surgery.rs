...
impl Manifold for TorusSurgery {
    fn tight(&self) -> i64 {
        -2
    }
}

pub fn tight_torus_surgery(p: i64, q: i64, a: i64, b: i64) -> i64 {
    ...
    assert!(p != 1, "({},{})-torus knot is an unknot. Enter a non-trivial torus knot!",p,q);

    // r = infinity => S^3
    if r.is_zero() {
        println!("({},{}) torus knot, surgery coefficient = {}/{}, tight contact structures: {}", p, q, a, b, 1);
        1
    } // 0-surgery on trefoils
    else if (qp == Fraction::new(3,2) || qp == Fraction::new(-3,2)) && r==0 {
        println!("({},{}) torus knot, surgery coefficient = {}/{}, tight contact structures: infinity", p, q, a, b);
        -1
    } else if r >= pq {
        let tuples = manifold.surgery_above_pq();
        let surgery = tuples.0 + tuples.1 + tuples.2;
        println!("({},{}) torus knot, surgery coefficient = {}/{}, tight contact structures: {}", p, q, a, b, surgery);
    //    println!("From non-loose: {}. From tight: {}. From non-thickenable: {}", tuples.0, tuples.1, tuples.2);
     //   println!("normalized Seifert fibration = ...  zero-twisting: {}. negative-twisting: {}.", 0, surgery);
        surgery
    } 
      
    else if r >= pq - 1 && r < pq {
        let tuples = manifold.surgery_above_pq_1();
        let surgery = tuples.0 + tuples.1 + tuples.2;
        println!("({},{}) torus knot, surgery coefficient = {}/{}, tight contact structures: {}", p, q, a, b, surgery);
   //     println!("From non-loose: {}. From tight: {}. From non-thickenable: {}", tuples.0, tuples.1, tuples.2);
        ...
        /*
        let tuples = surgery_below_pq(qp, r);
        let surgery = tuples.0 + tuples.1 + tuples.2;
        println!("TEST({},{}) torus knot, surgery coefficient = {}/{}, tight contact structures: {}", p, q, a, b, surgery);
        println!("From non-loose: {}. From tight: {}. From non-thickenable: {}", tuples.0, tuples.1, tuples.2);
        surgery  */
    // r < pq-1
    }  
    else {
        let tuples = manifold.surgery_below_pq();
...
//        println!("From non-loose: {}. From tight: {}. From non-thickenable: {}", tuples.0, tuples.1, tuples.2);
 //       if qp.is_negative() {
    //        println!("normalized Seifert fibration = ...  zero-twisting: {}. negative-twisting: {}.", tuples.0 + tuples.1, tuples.2);
  //      } else {
   //         println!("normalized Seifert fibration = ...  zero-twisting: {}. negative-twisting: {}.", tuples.0, tuples.1 + tuples.2);
//        }
